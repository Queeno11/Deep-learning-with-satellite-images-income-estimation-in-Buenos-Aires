...
def get_areas_for_evaluation():
    a_graficar = {
        # Test area 2
        "VILLA_31": [
            [-58.3942356256, -34.5947320752],
            [-58.3679285222, -34.5947320752],
            [-58.3679285222, -34.5760771024],
            [-58.3942356256, -34.5760771024],
            [-58.3942356256, -34.5947320752],
        ],
        "VILLA_21_24": [
            [-58.4108331428, -34.6551111569],
            [-58.3944394813, -34.6551111569],
            [-58.3944394813, -34.6437196838],
            [-58.4108331428, -34.6437196838],
            [-58.4108331428, -34.6551111569],
        ],
        "ADROGUE_CHICO": [
            [-58.409929469, -34.817989341],
            [-58.3966363097, -34.817989341],
            [-58.3966363097, -34.8070532351],
            [-58.409929469, -34.8070532351],
            [-58.409929469, -34.817989341],
        ],
        # Test area 1
        "PARQUE_LELOIR": [
            [-58.67620171, -34.6352316984],
            [-58.6650723329, -34.6352316984],
            [-58.6650723329, -34.6252500848],
            [-58.67620171, -34.6252500848],
            [-58.67620171, -34.6352316984],
        ],
        "BELLAVISTA": [
            [-58.7013553135, -34.5908167952],
            [-58.6871646388, -34.5908167952],
            [-58.6871646388, -34.5802173929],
            [-58.7013553135, -34.5802173929],
            [-58.7013553135, -34.5908167952],
        ],
        "NORDELTA": [
            [-58.6856032553, -34.4167915279],
            [-58.6720706202, -34.4167915279],
            [-58.6720706202, -34.4065712602],
            [-58.6856032553, -34.4065712602],
            [-58.6856032553, -34.4167915279],
        ],
        "NORDELTA2": [
            [-58.6649109046, -34.4174228815],
            [-58.6445404177, -34.4174228815],
            [-58.6445404177, -34.4011242703],
            [-58.6649109046, -34.4011242703],
            [-58.6649109046, -34.4174228815],
        ],
    }
    # Fix issue with Adrogue chico: doesnt fit fully on any dataset
    a_graficar["ADROGUE_CHICO"] = [
        [x, y + 0.004] for x, y in a_graficar["ADROGUE_CHICO"]
    ]
...
def ds_plot_example(ds, poly, ax):
    ...
    size_mask = np.ma.masked_where(
        size_mask == 0, size_mask
    )  # This is a 20x20 array with all values masked
    ax.imshow(
        size_mask, extent=poly_extent
    )  # Plots the area of the hole poly (size_mask has nothing to plot)
...
def generate_grid(
    ...
    if generate:
        ...
        # Cargo modelo
        model_path = (
            f"{path_dataout}/models_by_epoch/{savename}/{savename}_{best_epoch}"
        )
        ...
        # Guardo la grilla
        grid_preds.to_parquet(
            rf"{grid_preds_folder}/{savename}_{best_epoch}_predictions_{year}.parquet"
        )
    else:
        print(
            f"Abriendo ",
...
def plot_grid(grid_preds, modelname, year=2013):
    ...
    AMBA = [
        [-58.6715814736, -34.7982854506],
        [-58.2193109518, -34.7982854506],
        [-58.2193109518, -34.4570175785],
        [-58.6715814736, -34.4570175785],
        [-58.6715814736, -34.7982854506],
    ]
    ...
    bins = gdf_plot_example(
        icpag, var="var", poly=poly, ax=axs[1], alpha=1
    )  # , vmin=icpag["ln_pred_inc_mean"].quantile(.1), vmax=icpag["ln_pred_inc_mean"].quantile(.9))
    ...
    os.makedirs(savepath, exist_ok=True)
    plt.savefig(
        f"{path_outputs}/{modelname}/{modelname}_amba_{year}.png",
        bbox_inches="tight",
        dpi=300,
    )
    print(
        "Se creó la imagen: ", f"{path_outputs}/{modelname}/{modelname}_amba_{year}.png"
    )
...
if __name__ == "__main__":
    ...
    datasets, extents, df = main.open_datasets(
